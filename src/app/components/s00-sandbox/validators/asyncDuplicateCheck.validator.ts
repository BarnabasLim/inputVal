...
export function asyncDuplicateCheck(control:AbstractControl):Promise<ValidationErrors|null>{
    let takenName=['super','cat','bob']

    return new Promise((resolve,reject)=>{
        setTimeout(()=>{
            if(takenName.includes(control?.value?.toLowerCase())){
...
// export function asyncDuplicateCheck1(control:AbstractControl):Promise<ValidationErrors|null>{
